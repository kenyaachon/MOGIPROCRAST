...
/*
 * Copyright (C) Open Trivia Database
 *
...
public class TriviaAPI {
    ...
    private class GetTriviaDB extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            //Retrieve category chosen for trivia questions
            SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(kioskService);
            String syncConnPref = sharedPref.getString("trivia_categories", "Any");
            Log.i("Trivia Categories", syncConnPref);
        ...
        protected void onPostExecute(Void result) {
            ...
            if(response.trim().equalsIgnoreCase("SocketTimeoutException".trim())) {
                ...
                //Check to make sure database sends questions, if not load offline Database
                if(triviaQuestionArrayList.size() == 0){
                    kioskService.displayQuestions(getOfflineDB());
                } else{
