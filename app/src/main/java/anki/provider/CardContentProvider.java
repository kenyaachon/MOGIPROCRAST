...
import com.iruss.mogivisions.procrastimatev1.BuildConfig;
...
import anki.FlashCardsContract;
import anki.FlashCardsContract.CardTemplate;
...
import static anki.FlashCardsContract.READ_WRITE_PERMISSION;

//import anki.BuildConfig;
    ...
    private int bulkInsertNotes(ContentValues[] valuesArr, long deckId) {
        ...
        try {
            ...
            for (int i = 0; i < valuesArr.length; i++) {
                ...
                // Create empty note
                libanki.Note newNote = new libanki.Note(col, model); // for some reason we cannot pass modelId in here
                // Set fields
                // Check that correct number of flds specified
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case NOTES: {
                ...
                String tags = values.getAsString(FlashCardsContract.Note.TAGS);
                // Create empty note
                libanki.Note newNote = new libanki.Note(col, col.getModels().get(modelId));
                // Set fields
                String[] fldsArray = Utils.splitFields(flds);
                // Check that correct number of flds specified
                if (fldsArray.length != newNote.getFields().length) {
